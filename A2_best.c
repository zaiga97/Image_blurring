...
unsigned short int blur(const int xc, const int  yc, const int sblockcols, const int k, const double const ker[], const int ker_s, const unsigned short int const image[])
{
	int inizio = (yc - ker_s)*sblockcols + (xc - ker_s);
	double sum = 0;
	...
	for (int i = 0; i < k; ++i) //indice di riga
	{
		...
		{
			sum += ker[(i*k + j)] * image[(inizio + (i * sblockcols) + j)];
		}
	}
	//printf("%d \n", (int)sum/ker_weight);
	return sum;
...
void halo_exc(unsigned short *b, int ker_s, int blockrows, int blockcols, int sblockrows, int sblockcols, MPI_Comm GRID_COMM_WORLD)
{
	int source;
  	int dest;
  	MPI_Request request;
    ...
    MPI_Type_vector(blockrows, ker_s, sblockcols, MPI_UNSIGNED_SHORT, &vsblocktype2);
    ...
    MPI_Type_vector(ker_s, sblockcols, sblockcols, MPI_UNSIGNED_SHORT, &osblocktype2);
    ...
    {
    	MPI_Irecv(&b[ker_s*sblockcols], 1, vsblocktype, source, 1, GRID_COMM_WORLD, &request);
    }
    else
    {
    	for (int i = ker_s; i < blockrows + ker_s; ++i)
    	{
    ...
    {
    	MPI_Send(&b[ker_s*sblockcols + blockcols], 1, vsblocktype, dest, 1, GRID_COMM_WORLD);
    }
    ...
    if (source != MPI_PROC_NULL)
    {
    	MPI_Irecv(&b[ker_s*sblockcols + ker_s +blockcols], 1, vsblocktype, source, 2, GRID_COMM_WORLD, &request);
    }
    else
    {
    	for (int i = ker_s; i < blockrows + ker_s; ++i)
    	{
    		for (int j = ker_s + blockcols; j < 2*ker_s + blockcols; ++j)
    		{
    			b[i*sblockcols + j] = 0;
    		}
    	}
    ...
    if (dest != MPI_PROC_NULL)
    {
    	MPI_Send(&b[ker_s*sblockcols + ker_s], 1, vsblocktype, dest, 2, GRID_COMM_WORLD);
    }
    ...
    else
    {
    	...
    	{
    		for (int j = 0; j < 2*ker_s + blockcols; ++j)
    		{
    			b[i*sblockcols + j] = 0;
    		}
    	}
    ...
    if (source != MPI_PROC_NULL)
    {
    	MPI_Irecv(&b[(blockrows + ker_s)*sblockcols], 1, osblocktype, source, 4, GRID_COMM_WORLD, &request);
    }
    else
    {
    	for (int i = (ker_s+blockrows); i < 2*ker_s+blockrows; ++i)
    	{
    		for (int j = 0; j < 2*ker_s + blockcols; ++j)
    		{
    			b[i*sblockcols + j] = 0;
    ...
    if (dest != MPI_PROC_NULL)
    {
    	MPI_Send(&b[ker_s*sblockcols], 1, osblocktype, dest, 4, GRID_COMM_WORLD);
    }
...
int main(int argc, char** argv)
{
	...
	else
	{
		...
		if (ker_t == 1)
		{
			...
			else
			{
				...
				sprintf(output_name, "%s.b_%d_%dx%d_%3f.pgm",image_name, ker_t, k, k, ker_f);
			}
		}
		...
		else
		{
			...
			{
				output_name = (char *)malloc(100 * sizeof(char));
				sprintf(output_name, "%s.b_%d_%dx%d.pgm",image_name, ker_t, k, k);
			}
		}
	}
	...
	unsigned int size;
  	...
  	int sblockrows = ysize/nprow + 2*ker_s;
  	int sblockcols = xsize/npcol + 2*ker_s;

  	unsigned short int *local_image = (unsigned short int*)malloc( sblockrows * sblockcols * 2 * sizeof(char));
...
// inizialize right kernel
	...
	*/
	...
	MPI_Scatterv(image, counts, disps, blocktype, &local_image[ker_s * sblockcols + ker_s], blockrows*blockcols, sblocktype, 0, GRID_COMM_WORLD);
	short unsigned int* local_blurred_image;
	local_blurred_image = (short unsigned int *)malloc(blockcols * blockrows * sizeof(short int));

	MPI_Barrier(GRID_COMM_WORLD);
	halo_exc(local_image, ker_s, blockrows, blockcols, sblockrows, sblockcols, GRID_COMM_WORLD);
	...
	#pragma omp parallel //proc_bind( master )  //Perform the blur only of the small block
		{
			...
			for (int j = 0; j < blockrows; ++j)
			{
				...
				{
					local_blurred_image[i + j*blockcols] = blur(i + ker_s, j + ker_s, sblockcols, k, ker, ker_s, local_image);
				}
			}
		}
